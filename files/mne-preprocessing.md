...
これはICA[^ica]という方法で波を幾つかの波に分け、
その上で眼球運動や心電図っぽい波を除去するフィルタとします。

[^deep]: DeepLearningが今の世の流行りですが、DeepLearningは画像が得意だという点があります。これはCNNという手法があるからですが、波形をCNNに適用する手法もあります。でも、これが筋が良いかと言うと色々意見もあろうかと。ICAを使うところでしょう。

[^ica]: 日本語で独立成分分析。これはPCA(主成分分析)の発展系みたいなもので、分散を使って特徴量を分けていく古典的機械学習の手法です。
