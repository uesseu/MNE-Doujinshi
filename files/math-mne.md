...
何故これの対角行列かと言うと、
Xの1行目…例えばチャンネル1としましょう。
これとXの2列目…これはチャンネル2です。これらの掛け算は
チャンネル間の相関を表しているのであって、振幅を表しません。
...
知らないカスはググれ。
僕はこんな同人誌を書く程度には親切で友達がいっぱい出来そうな人なので教えてあげます。
...
XはまぁXなんですが、WXは「おおむねこのくらいなんじゃねえかな？」的な値です。
WがIでも計算は出来るのですが、WがIの場合どうなるかと言うと
「なるべく全ての概ねが、平等にIに近づくように」です。
Wが、例えばXの振幅の予測値ならば
「なるべくXの大きすぎる所を小さく」になるわけです。
...
$W^TW$が長いので$\Sigma$って書き換えさせて下さい。
$$X = (A^TA + \lambda \Sigma)^{-1}A^TY$$
出ました！MNEの式です！だけど、なんだか書いてあるのと違う？
お前は教科書に載っている通りじゃないと満足しない心の狭い人間なのだね？
OK、書き直すわ…これがめんどいの！
$$(AB)^{-1}=B^{-1}A^{-1}$$
この公式を使います。
$$X = (A^TA(I + (A^TA)^{-1}\lambda \Sigma)^{-1}A^TY$$
$$X = (A(I + (A^TA)^{-1}\lambda \Sigma)^{-1}(A^T)^{-1}A^TY$$
$$X = (A(I + (A^TA)^{-1}\lambda \Sigma)^{-1}Y$$
$$X = (A + A(A^TA)^{-1}\lambda \Sigma)^{-1}Y$$
$$X = ((A\Sigma^{-1} + (A^T)^{-1}\lambda)\Sigma)^{-1}Y$$
$$X = \Sigma^{-1}(A\Sigma^{-1} + (A^T)^{-1}\lambda)^{-1}Y$$
$$X = \Sigma^{-1}((A\Sigma^{-1}A^{-1} + \lambda)(A^T)^{-1})^{-1}Y$$
$$X = \Sigma^{-1}A^T(A\Sigma^{-1}A^T + \lambda I)^{-1}Y$$
これやこの、MNEの式です。
だけど、まだ$\lambda I$が怪しいですね？
ここで、はじめの式を見て下さい。
$$argmin L_{x} =  {||Y - AX||}^2 + \lambda {||WX||}^2$$
これ、あからさまに第一項がノイズ、第二項がデータですね？
ってことは$\lambda$というのは実は
ノイズの2乗に比例する数値ならリーズナブルということになります。
というか、実際の所、$\lambda I$って
どんだけノイズが入っているかであるべきです。
そして、そのノイズはチャンネルによって違います。
だから、脳内のノイズの分散共分散行列をCとして、
$\lambda I$の代わりとしてあげます。
…ノイズと言いましたが「背景」と言ったほうがわかりやすいかも知れません。

実際計算してもちゃんとなります。(書きませんが、手元でなりました)
$$X = \Sigma^{-1}A^T(A\Sigma^{-1}A^T + C)^{-1}Y$$
で、この値は正則化されていないわけですから、
グニョングニョンの値になってしまいます。
なので、結局正則化のパラメータ入れるのです...。
(ここは流石にググって)

$$X = \Sigma^{-1}A^T(A\Sigma^{-1}A^T + \lambda^2 C)^{-1}Y$$
...
ぐぐｒ…これは、それぞれがどのくらい相関しているかの行列です。
実際は相関の2乗を集めた的な行列です。これをCとします。
ってことは、$C^{-1/2}X$とするとそれぞれ独立という感じになるはずです。
ですが、$Y = AX$という等式は崩せないので、Aをいじります。
具体的には$Y = AX$という等式は崩せないので、Aをいじります。
$$Y = AC^{1/2}C^{-1/2}X$$
...
こんなかんじ。
$$ \hat{A^{\dagger}}C^{1/2} $$
$$ = \Sigma^{-1}A^TC^{1/2}(AC^{1/2}\Sigma^{-1}C^{1/2}A^T + C)^{-1}C^{1/2}$$
$$ = \Sigma^{-1}A^T(A\Sigma^{-1}A^T + \lambda ^2 I)^{-1}Y$$
...
なら、チャンネル数じゃなくてXのソース数分の大きさの行列にするべきです。
...
$$X = \Sigma^{-1}A^T(A\Sigma^{-1}A^T + \lambda ^2 I)^{-1}C^{-1/2}Y$$
sLORETAによると$||A^\dagger A||$がソースの分散でした。
だから、まずは
$$X = \Sigma^{-1}A^T(A\Sigma^{-1}A^T + \lambda ^2 I)^{-1}Y$$
この式の内容から考えていきます。
sLORETAで想定される分散はこうなります。
$$ \Sigma^{-1}A^T(A\Sigma^{-1}A^T + \lambda ^2 I)^{-1}A$$
$\Sigma$はソースの分散みたいなものですね。(こんな風になるんじゃねえの？なWの二乗)
