...
脳の解析は難しいのですが、技術的な入門書、特に和書に乏しい現状があります。
僕は「知識は体で覚えるもの」と信じていますので、理論だけでは足りないと思っています。
(もちろん、理論も無いと駄目です。僕が理論がまだまだ弱く、ダメです。)
でも、和書少なすぎです。だから技術系の同人誌を書くことにしました。
本書では脳磁図、脳波、MRI解析を「体で覚える」べく実践していきます。

…というか、僕のモチベーション維持のための本です。一人じゃ辛い。

## 本書の目的・限界・対象者・使い方
MNE/pythonやfreesurferを用いて脳内の電源推定…特にソースベース解析を行うための
解析環境の構築と解析の基礎を概説します。可能な限り効率的な解析環境を構築し、楽をします。
僕が個人的に考えている事もちょくちょく書きます。
また、MNEpythonやfreesurferの使い方ついて少し説明をします。
あくまで上記が主題ですが、freesurferによる画像研究についても少しだけ触れます。
...
(関数の集まり？)です。MNEは元来minimum norm estimation(MNE)を実装するために
開発されましたが、開発が進んで、それ以外の解析方法を取ることも出来るフレームワークとなり、
現在は自由度が非常に高いです。(引き換えに難易度が一寸高いです。)
wavelet変換、コネクティビティ、その他あらゆる事が出来ます。出来るのですが…
使いこなすためには深い生理学の知識が必要です。
...
その上、違うCPU使ったら結果が変わる~~トンデモ仕様~~個性あるソフトです。
...
|freefurfer/freeview|MRI画像処理ソフト|photoshop,頭はいいが仕事の遅い部下|
...
メモリ＞CPU＞GPUと思います。
freesurferはOSやCPUが変わったら結果が変わるという
~~トンデモ~~ 仕様がありますから一つ「このコンピュータを使う」と決めちゃう必要があります。
...
素のjupyterでも十分ですが、折角なので拡張しておきましょう。ターミナルで下記を叩いてください。
...
僕の理解力的に解説はとてもじゃないけれど無理なので、しません。
また、データ解析の常識は進化が速いうえにその手の論文を
網羅的に読めていないと正確なところは書けません。
...
脳波の場合はモンタージュや眼球運動チャンネルの設定があります。

例えば、僕がやったときは実験時に64番から72番チャンネルが眼球運動に割り当てられていました。
...
epochは元データのキーとなる場所に特定の印を付けたものです。
例えば、特定の刺激を提示した時などに元データ(raw)に
刺激用チャンネルとしてアナログ信号を記録することが出来ますが、
それをデジタル的な印にしたものですね。
evokedは特定の印がついた所だけ切り出して加算平均したものです。
...
```
ちなみに、このmethon=autoというのはMNEに実装された新しいやり方だそうです。
...
[^gekiomo]:これは激重です。何故なら306チャンネルのMEGからソースに落とし込むと計算方法によっては10000チャンネルくらいになります。ROIを絞ったとしても「疾患対照群の人数×タスクとプラセボの組み合わせ×ROIの数×EPOCHの数」回解析しないといけなくなります。さらに全ての波形をwavelet変換してpowerとitcに落とし込むのですから…途方もない計算量です。これがあるため、ソースベース解析にはコンピュータ・クラスタやワークステーションが必要なのです。
...
snr = 3.
lambda2 = 1. / snr ** 2
stcevo = mne.minimum_norm.apply_inverse(evoked,
  inverse_operator, lambda2,
  method=method, pick_ori=None)
info = mne.create_info(ch_names=['MEG1'],
  ch_types=['grad'],sfreq= 1000)
evoked_array = mne.EvokedArray([stcevo.data[0]],
  info, -0.2,nave=stcevo.data.shape)
freqs=np.arange(30,100,1)
evoked_array_power=mne.time_frequency.tfr_morlet(evoked_array,n_jobs=4,
  freqs=freqs, n_cycles=6, use_fft=True,
                        return_itc=False, decim=1)
evoked_array_power.apply_baseline(baseline=(0.1,None),mode='zscore')
```
ソースレベルevoked_powerです…。
どうも無理矢理感あります…正しいかどうか分かりません。
<!--
## ソースレベル解析〜3コネクティビティ
ぼくは　さんすうがわからないから　いまはまだ　これいじょうは　むりです。
-->
...
初心者には結構難しいです。だから、ここでは物凄くざっくり(やや不正確な)解説をします。
...
しかし、sinもcosも未来永劫絶対に減衰しない波であるため、フーリエ変換はきついです。
...
$$f(x)=a_0+\sum_{k=1}^{\infty}(a_n\cos{\frac{2\pi ntx}{T}}+b_n\sin{{\frac{2\pi ntx}{T}}})$$
...
なんとpythonなんかは$e^i$を計算できたりします。凄いですね！！！

というわけで、上の式をそのままpython流に書けばフーリエ変換は
自分でコーディングすることが出来るわけです。
でも、僕は自分ではしないです。
...
この2つを我々の領域ではそれぞれ
power、phaselocking_factorと呼びます。
...
- Analyzing Neural Time Series Data:Theory and Practice
 ...
 amazonでも売ってます。高いですが是非買ってください。
 通読はきついですが、どうすれば良いのかわからなくなった時の道標です。
 MNE-Pythonはどうすれば良いのか分からなくなる事が多いのです。

- 事象関連電位―事象関連電位と神経情報科学の発展
 和書の名著です。なのですが、絶版です。古本を見つけたらすかさず買いましょう。
 内容的には臨床医学に近い内容で、数学の要素は薄い文系の本です。
 通読向けではありますが、やはり手を動かしながらじゃないときついです。

- 脳磁気科学-SQUID計測と医学応用
 これも絶版の和書です。古本見つけたら是非買ってください。僕は算数も英語が苦手なのですが、
 ...
 - 逆問題の考え方　結果から原因を探る数学
 これは逆問題についての数学的な本です。信頼と安心のブルーバックスで安価です。
 電子書籍にもなっていて入手難度は0です。買いましょう。
 内容は数学ですが、文体がどうも酔っ払いながら書いたんじゃねえかと思われる変な文体です。
 でも、内容は初心者にも分かりやすく、素晴らしいです。
 逆問題に関して書かれており、脳磁図の解析の事について理解を深めるのに良いです。
...
このウェーブレット変換スライドは素晴らしいです。
作者が学生の頃に勉強して作ったのだそうですが、
多くの「分かっている人向けの数学的入門書」と違い、直感的に分かるように
書いてあるのが何より素晴らしく、必見です。

# おすすめSNS
qiita
http://qiita.com
日本のプログラマ用のSNSです。かなり分かりやすい記事が多く、大変重宝します。
