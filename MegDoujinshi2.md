...
やはり戦闘力ではプログラマの足元にも及ばないのです。
しかし、MNEpythonはプログラマのプログラミング程
難しくないはずです。この領域は
プログラマになる必要はないのですが、
生理学を理解しないといけないのでそっちのほうがつらいです。
```
...
出来るだけ理解の範囲で書きます。ここからは線形代数の範囲になります。
最低限…最小二乗法くらいは勉強してください。
...
この式に更にノイズを考慮した変数をや重み付け入れたりしていったら
Minimum-norm-estimationの式になるというわけです。凄いですね！

お前は次に「めんどい。何故最小二乗法じゃダメなんですか？」という。
最小二乗法ではいけません。何故なら、最小二乗法で導出した場合
解が無限に生じてくるからです。
...
凄いですね！これやこの、MNEの式です。
重みは何に使うかですが、脳の深いところよりも浅い所にMNEは偏っています。
なので、その偏りを補正してあげるわけです。
補正のパラメータは色々あるかと思います。
MNEpythonでは補正のパラメータを作る関数が用意されており、
それを使って作ることになるでしょう。

また、ノイズ(ここではcで表す)まで考慮すると
$$w^{-1}A^T(Aw^{-1}A^T+c)^{-1}Y=X$$
という風な式になります。
(この部分にはベイズの統計学が必要で、僕はまだ出来ていません…)
